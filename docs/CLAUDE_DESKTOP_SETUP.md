...
This guide explains how to configure Claude Desktop to use the MCP Resource Server for file and image operations with blob storage support.
...
1. Install the MCP server: `cd /opt/src/mcp/mcp_resource_server && uv sync`
...
| `RESOURCE_SERVER_BLOB_STORAGE_ROOT` | `/mnt/blob-storage` | Directory for shared blob storage |
| `RESOURCE_SERVER_BLOB_STORAGE_MAX_SIZE_MB` | `100` | Maximum file size for blob storage |
| `RESOURCE_SERVER_BLOB_STORAGE_TTL_HOURS` | `24` | Time-to-live for stored blobs |
...
- **get_image_resource**: Store resized image in shared blob storage
...
4. **Blob Storage**: Use `get_image_resource` or `get_file_resource` when:
...
2. Add the following files from the MCP Resource Server repository:
   ...
   - `CLAUDE.md` - Technical context and debugging info
   - `.env.example` - Configuration reference (if exists)

## Troubleshooting

### Checking Logs

Claude Desktop logs are located at: `/workspace/claude-desktop-logs`

The MCP Resource Server logs specifically are in: `mcp-server-mcp-resource-server.log`

```bash
# View recent errors
tail -100 /workspace/claude-desktop-logs/mcp-server-mcp-resource-server.log

# Watch logs in real-time
tail -f /workspace/claude-desktop-logs/mcp-server-mcp-resource-server.log
```
...
**Solutions**:
1. Check the server runs standalone:
   ```bash
   cd /opt/src/mcp/mcp_resource_server
   uv run mcp-resource-server
   ```
...
3. Check logs for Python errors:
   ```bash
   grep -i error /workspace/claude-desktop-logs/mcp-server-mcp-resource-server.log
   ```
...
**Solutions**:
1. Verify the file exists at the resolved URL:
   ```bash
   # For file:// protocol
   ls -l /mnt/resources/your_file_id
   ```

2. Check the `RESOURCE_SERVER_URL_PATTERN` is correct
...
1. Verify the file is a valid image format (JPEG, PNG, GIF, WebP)
   ```bash
   file /mnt/resources/your_image_id
   ```

2. Check the image isn't corrupted:
   ```bash
   uv run python -c "from PIL import Image; Image.open('/mnt/resources/your_image_id').verify()"
   ```
...
**Solutions**:
1. Check blob storage directory exists and is writable:
   ```bash
   ls -ld /mnt/blob-storage
   mkdir -p /mnt/blob-storage
   chmod 755 /mnt/blob-storage
   ```
...
3. Check the file size doesn't exceed `RESOURCE_SERVER_BLOB_STORAGE_MAX_SIZE_MB`

4. Review blob storage configuration in environment variables
...
sudo chmod 755 /mnt/resources
sudo chmod 755 /mnt/blob-storage

# Or change ownership
sudo chown $USER:$USER /mnt/resources
...
1. Configure all MCP servers to use the same `RESOURCE_SERVER_BLOB_STORAGE_ROOT`
2. Use `get_image_resource` or `get_file_resource` to store files
3. Share the returned `resource_id` (format: `blob://TIMESTAMP-HASH.EXT`)
4. Other servers can access the file directly from the blob storage directory
...
```json
{
  "mcpServers": {
    "mcp-resource-server": {
      "command": "uv",
      "args": ["--directory", "/opt/src/mcp/mcp_resource_server", "run", "mcp-resource-server"],
      "env": {
        "RESOURCE_SERVER_BLOB_STORAGE_ROOT": "/mnt/shared-blobs"
      }
    },
    "other-mcp-server": {
      "command": "uv",
      "args": ["--directory", "/opt/src/mcp/other_server", "run", "other-server"],
      "env": {
        "BLOB_STORAGE_ROOT": "/mnt/shared-blobs"
      }
    }
  }
}
...
- Check server logs: `/workspace/claude-desktop-logs/mcp-server-mcp-resource-server.log`
- Review documentation: `README.md`, `CLAUDE.md`
- Test server standalone: `uv run mcp-resource-server`
- Verify configuration: `claude_desktop_config.json`

## Related Documentation

- [MCP Resource Server README](../README.md)
- [MCP Resource Server CLAUDE.md](../CLAUDE.md)
